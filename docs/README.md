...
- **Gestor de frota —** Principal perfil operacional e decisório. É o servidor responsável pela gestão do ciclo de vida dos veículos. Utiliza o sistema para cadastrar a frota, autorizar manutenções e monitorar indicadores de desempenho. Requer uma interface analítica, com acesso centralizado a alertas de vencimentos e relatórios gerenciais para suporte à tomada de decisão.
- **Motorista —** Usuário operacional responsável pela condução e zelo pelos veículos. Sua interação com o sistema foca no registro de ocorrências e na solicitação de manutenções corretivas, enviando descrições detalhadas e evidências fotográficas. Exige uma interface extremamente simplificada e intuitiva, adequada a diferentes níveis de familiaridade tecnológica.
- **Setor administrativo —** Atua exclusivamente na camada de governança e conformidade do sistema. É responsável pelo cadastro dos perfis e pela validação técnica da documentação inserida. Sua função é garantir que apenas gestores autorizados tomem decisões e que os documentos dos veículos estejam em conformidade legal antes de sua liberação
- **Solicitante de veículo —** Servidor que demanda deslocamentos para atividades institucionais (vistorias, serviços externos, etc.). Utiliza o sistema para realizar reservas, informando destino e período. Seu foco é a agilidade no preenchimento e a transparência no acompanhamento do status de aprovação do seu pedido.
